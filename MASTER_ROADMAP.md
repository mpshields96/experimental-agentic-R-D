...
| Sport | API Key | Status | Priority |
|---|---|---|---|
| Tennis (ATP) | tennis_atp | ✅ COMPLETE (S15+S16): kill switch + dynamic discovery via fetch_active_tennis_keys() | DONE |
| Tennis (WTA) | tennis_wta | ✅ COMPLETE (S15+S16): same — included in fetch_active_tennis_keys() | DONE |
| College Baseball | baseball_ncaa | ❌ REJECTED: thin book coverage, no probablePitcher on sportId=22 | Rejected |
...
## SECTION 10: Skills System (Session 36, 2026-02-26)

Claude Code personal skills live in `~/.claude/skills/`. These are on-demand
invocation guides — not always-loaded like CLAUDE.md — used for specific workflows.
...
## SECTION 11: Session Summary (S19–S36)

| Session | Key Deliverable | Tests |
|---|---|---|
| S19–S21 | Credit guards, PDO regression signal (NBA), architecture audit | 933 |
| S22 | NBA PDO kill switch (66 tests), compute_pdo(), pdo_kill_switch() | 933 |
| S23 | KOTC analyzer (74 tests), injury sidebar, V36→V37 bridge | 1011 |
| S24 | Governance (CLAUDE.md), backup.sh, credit guards, V37 two-AI system | 1011 |
| S27 | Grade tier pipeline (A/B/C/Near Miss), go-live config, calibration gate | 1103 |
| S29 | Math audit: totals multi-line consensus bug FIXED, RLM direction bug FIXED, dead code deleted | 1079 |
| S33 | CST game times, Book Coverage page, guide rewrite | 1079 |
| S34 | Analytics gate text, Pinnacle removed, docstring improvements | 1079 |
| S35 | PropCandidate + parse_props_candidates() + edge/grade UI (page 08) | 1154 |
| S36 | titanium-session-wrap skill, MASTER_ROADMAP housekeeping | 1154 |

**Note:** Sessions S25–S26 and S28–S32 are documented in SESSION_LOG.md. This table
covers major deliverable sessions only.
